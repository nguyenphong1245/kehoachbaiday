...
- **Khóa lớp**: Khi một lớp đã sửa xong và muốn đảm bảo không bị sửa nhầm khi sửa các lớp khác, em thực hiện khóa lớp lại bằng cách: nháy chuột vào lớp muốn khóa trong hộp thoại Layer > Điểm ánh Vị trí và kích thước lớp rồi nháy chuột vào thuộc tính mà em muốn khóa (Hình 28.1).

![Hình 28.1. Các thuộc tính khóa](Hình_28.1.jpg)
...
![Hình 28.2](Hình_28.2.jpg)

### Bài tập và câu hỏi
1. Hãy mô tả các thao tác cần thực hiện để mở một tệp ảnh làm lớp trong GIMP.
2. Giải thích cách khóa một lớp ảnh và lý do tại sao cần phải khóa lớp.
3. Khi nào thì nên gộp lớp trong quá trình chỉnh sửa ảnh?

### Hình ảnh mô tả
- Hình 28.1: Các thuộc tính khóa lớp trong GIMP.
- Hình 28.2: Ví dụ về các lớp ảnh trong GIMP.
...
Mỗi khi phải cài đặt một phần mềm mới trên điện thoại hay máy tính; trong thời gian chờ đợi em sẽ thấy có hình vòng tròn quay; hồ cát hoặc một thanh chạy tăng dần. Hãy phân tích một trong các biểu tượng để xem trong mỗi ảnh ta cần những thành phần nào?
...
Ngoài ra, GIMP còn cung cấp một số hiệu ứng để làm ảnh sinh động hơn: Để thêm hiệu ứng cho ảnh, em chọn **Filters > Animation** và chọn trong các hiệu ứng có sẵn, ví dụ hiệu ứng **Bend**.
...
### Bài tập và câu hỏi
1. Phân tích các biểu tượng mà em thấy khi cài đặt phần mềm mới.
2. Giải thích cách thiết lập thời gian cho các khung hình trong ảnh động.
3. Nêu các bước để xuất tệp ảnh động trong GIMP.



# Blend (Hiệu ứng chuyển động mờ dần giữa các layer)

## Nội dung lý thuyết
Hiệu ứng này cần ít nhất 3 lớp, lớp dưới cùng là lớp nền; hình ảnh sẽ được chuyển dần dần từ lớp 2 lên lớp trên cùng. Giữa 2 khung hình tương ứng với 2 lớp ảnh gốc có một số khung hình trung gian (số lượng trong ô Intermediate frames) được tạo ra bằng cách hoà trộn lớp nguồn, lớp đích và lớp nền.
...
**Hướng dẫn:**
1. **Bước 1:** Chọn File -> New rồi nhập 500 vào hai ô Width và Height rồi nháy OK.
2. **Bước 2:** Chọn màu trắng cho màu nổi, chọn công cụ tô màu hoặc nhấn tổ hợp phím Shift+B rồi tô màu cho lớp vừa tạo.
3. **Bước 3:** Nháy nút phải chuột vào lớp vừa tạo rồi chọn New Layer, đặt tên lớp là Layer 1.
4. **Bước 4:** Chọn màu xám nhạt cho màu nổi (ví dụ dcdcdc) và màu xám đậm cho màu nền (ví dụ 7f7f7f).
5. **Bước 5:** Nháy chuột vào công cụ Rectangle Select Tool trong phần tuỳ chọn công cụ: nháy chọn vào ô Rounded corners, đặt Radius 30 và xác định vị trí vùng chọn: Position 235, 0; Size 30, 90.
6. **Bước 6:** Nhấn tổ hợp phím Shift + B rồi tô màu cho vùng chọn vừa tạo (Hình 28.6a).
7. **Bước 7:** Chọn Duplicate layer trên Layer 1.
8. **Bước 8:** Nháy chuột vào lớp ảnh mới (Layer 1 copy) rồi chọn công cụ Flip chọn Vertical trong tuỳ chọn công cụ rồi nháy chuột vào lớp ảnh Layer copy (Hình 28.6b).
9. **Bước 9:** Nháy nút phải chuột vào lớp Layer copy và chọn Merge Down.
10. **Bước 10:** Chọn Duplicate layer trên Layer.
11. **Bước 11:** Chọn lệnh Unified Transform Tool trong hộp công cụ (nhấn tổ hợp Shift T); giữ chuột bên ngoài khung hình và quay sao cho hình quay 3 bước (tương ứng với góc quay 450) rồi nháy vào nút Transform (Hình 28.6c).
12. **Bước 12:** Nháy nút phải chuột vào lớp Layer copy và chọn Merge Down.
13. **Bước 13:** Thực hiện lại các bước 10, 11, 12 với góc quay 900 để thu được phần còn thiếu (Hình 28.6d).

## Hình ảnh mô tả
- **Hình 28.4:** Danh sách lớp ảnh.
- **Hình 28.5:** Hình tròn tạo bởi các nét.
- **Hình 28.6a:** Hình ảnh sau khi tô màu cho vùng chọn.
- **Hình 28.6b:** Hình ảnh sau khi flip lớp ảnh.
- **Hình 28.6c:** Hình ảnh sau khi thực hiện quay 450 độ.
- **Hình 28.6d:** Hình ảnh sau khi thực hiện quay 900 độ.
...
# Tiêu đề bài học
Tạo ảnh động với GIMP

## Nội dung lý thuyết
Trong bài học này, chúng ta sẽ học cách tạo ảnh động bằng phần mềm GIMP. Ảnh động là một chuỗi hình ảnh được hiển thị liên tiếp để tạo ra hiệu ứng chuyển động. Chúng ta sẽ thực hiện một ví dụ cụ thể để hiểu rõ hơn về quy trình này.
